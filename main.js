...
// Colapsa estancias (reduce “telarañas” en patios)
function collapseStays(points){
  ...
  const commit = () => {
    ...
    if (durMin >= STAY_MIN_MIN && maxR <= STAY_RADIUS_M){
      // punto representativo (centroide)
...
function simplifyRDP(points, epsilonMeters = 5){
  ...
  const eps = epsilonMeters / 111320; // grados/metro aprox (lat,lng pequeños)
  const sq = x => x*x;
...
// Construye segmentos limpios desde filas cronológicas
function buildSegmentsFromRows(rows){
  ...
  // 2) colapsar estancias
  pts = collapseStays(pts);

  // 3) eliminar picos por ángulo irreal en distancias cortas
  if (pts.length > 2){
    const cleaned = [pts[0]];
    for (let i=1;i<pts.length-1;i++){
      ...
      if (ang >= ANGLE_SPIKE_DEG && smallMove && (vAB < MAX_SPEED_KMH && vBC < MAX_SPEED_KMH)){
        continue; // descarta b
      }
      cleaned.push(b);
    }
  ...
  // 4) segmentar por gap / velocidad + anti-jitter
  const segments = [];
  let cur = [];
  for (let i=0;i<pts.length;i++){
    ...
    if (dtMin > GAP_MINUTES || v > MAX_SPEED_KMH){
      ...
      if (d >= JITTER_M) cur.push(p); // ignora saltitos
    }
  }
  if (cur.length >= 2) segments.push(cur);
...
// ====== Dibujo del trazo por usuario (usa pipeline) ======
function updateUserPath(uid){
  ...
  if (!entry){
    const polys = segs.map(seg => L.polyline(seg.map(s=>[s.lat,s.lng]), { weight: 3, opacity: 0.9, color: '#f2c200' }).addTo(state.pathLayer));
    state.userPaths.set(uid, { segments: polys });
  } else {
    entry.segments.forEach(pl => state.pathLayer.removeLayer(pl));
    entry.segments = segs.map(seg => L.polyline(seg.map(s=>[s.lat,s.lng]), { weight: 3, opacity: 0.9, color: '#f2c200' }).addTo(state.pathLayer));
  }
}
...
// ====== Exportar KMZ del día (mismo pipeline) ======
async function exportKMZFromState(){
  ...
  byUser.forEach((rows, uid) => {
    ...
    // mismo pipeline del mapa:
    const segs = buildSegmentsFromRows(rows);

    for (const seg of segs){
      const coords = seg.map(s => `${s.lng},${s.lat},0`).join(' ');
      kmlBody += `
        <Placemark>
          <name>${name}</name>
